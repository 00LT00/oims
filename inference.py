...
coco_id_mapping = {
    1: 'femoralhead', 2: 'sacrum',}  # pyformat: disable 
...
def build_inputs(image_path_pattern: Text, image_size: Union[int, Tuple[int,
  ...
  return raw_images, tf.stack(images), tf.stack(scales),fs



def save_feat_fig(feats_out):
      index=0
      for f in feats_out:
        if len(f.shape) == 4:
          # feature num per layer
          num_f = f.shape[-1]

          # feature size
          size = f.shape[1] 

          display_grid = np.zeros((size, size * num_f))
          ...
          plt.figure(figsize=(scale * num_f, scale))
          plt.title('P'+str(index))
          plt.grid(False)
          plt.imshow(display_grid, aspect='auto', cmap='viridis')
          plt.savefig(str(index)+'.png', dpi=750, bbox_inches = 'tight')
          index+=1
      return
...
def write_res_to_txt(fname,prediction, output_dir):
  boxes = prediction[:, 1:5]
  classes = prediction[:, 6].astype(int)
  scores = prediction[:, 5]
  print(fname)
  for i,j in enumerate(scores):
    if j<0.8:
      break
    print(coco_id_mapping[classes[i]])
    line_content=fname+' '
    line_content+=str(round(scores[i],3))+' '
    line_content+=str(int(boxes[i][1]))+' ' 
    line_content+=str(int(boxes[i][0]))+' '
    line_content+=str(int(boxes[i][3]))+' '
    line_content+=str(int(boxes[i][2]))
    print(line_content)
    with open(output_dir+coco_id_mapping[classes[i]]+'.txt','a') as txt:
        txt.writelines(line_content)
        txt.writelines('\n')
        txt.close()
  return

def write_res_to_xml(fname, prediction, output_dir, input_dir):
  input_paths = input_dir.split('/')
  path = output_dir+input_paths[-2]+'.xml'
  ...
  if not os.path.exists(path):
    root = ET.Element('image-list')
    tree = ET.ElementTree(root)
  else:
    tree = ET.parse(path)
    root = tree.getroot()
    for i in root.findall('./image/[name="'+fname+'"]'): #检测重复
      return
  

  image = ET.SubElement(root,'image')
  image.append(ET.Element('name'))
  image.find('name').text = fname
  for i, j in enumerate(scores):
    if j <0.8: #置信度检测
      break

    cococlass = ET.SubElement(image, coco_id_mapping[classes[i]])
    xmin = ET.SubElement(cococlass, 'xmin')
    xmin.text = str(boxes[i][1])
    ymin = ET.SubElement(cococlass, 'ymin')
    ymin.text = str(boxes[i][0])
    xmax = ET.SubElement(cococlass, 'xmax')
    xmax.text = str(boxes[i][3])
    ymax = ET.SubElement(cococlass, 'ymax')
    ymax.text = str(boxes[i][2])
...
class InferenceDriver(object):
  ...
  def inference(self, image_path_pattern: Text, output_dir: Text, **kwargs):
    ...
    with tf.Session() as sess:
      ...
      raw_images, images, scales ,fs= build_inputs(image_path_pattern,
                                                params['image_size'])
      ...
      for i ,fname in enumerate(fs):
          # write_res_to_txt(fname,predictions[i],output_dir)
          write_res_to_xml(fname,predictions[i],output_dir, image_path_pattern)
